function [X,cost,update,error] = vmc_step(Xinit,sampmask,samples,options,Xtrue)
...
%Jan 31 SJQ Based on python code, make scalefac the matrix norm
scalefac = sqrt(max(sum(abs(Xinit).^2,'all'))); 
...
vote=zeros(size(X,2),1);
t_list = [ceil(niter/15), ceil(niter/15)*2, ceil(niter/15)*3, ceil(niter/15)*4, ceil(niter/15)*5];
...
for i=1:niter;
    ...
    if epsilon == 0 %project onto equality constraints       
        ...
        if nd > epsilon %need to project
            ...
            %X(sampmask2) = alpha*X(sampmask2)+(1-alpha)*samples2;
        end
