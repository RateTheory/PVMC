...
def uniquify_flist(list_of_floats, eps=EPS_FLOAT):
    ...
    if len(list2) > 1:
        for idx in range(1, len(list2)):
            ...
            if abs(current_float - previous_float) < eps:
                repeated.append(idx)
...
class Spline:
    ...
    def __init__(self, lx, ly, tan="findiff", tension=0.0):
        ...
        if self._tan not in ["findiff", "pchip"]:
            raise WrongVar(Exception)
        if self._tan == "findiff":
            self.slopes_findiff()
        if self._tan == "pchip":
            self.slopes_pchip()
    ...
    def interval(self, x):
        # first or last point
        if abs(x - self._x0) <= EPS_FSTRICT:
            return self._i0, True
        if abs(x - self._xn) <= EPS_FSTRICT:
            return self._im, True
        # outside limits
        if x < self._x0:
            return self._i0, False
        if x > self._xn:
            return self._im, False
        # in the range
    ...
    def tval(self, x):
        ...
        # get t in interval
        if not inrange and idx == self._i0:
            return idx, 0.0
        if not inrange and idx == self._im:
            return idx, 1.0
        # interval data
    ...
    def find_xtr(self, xtr="max"):
        ...
        for guess in guesses:
            xm, ym = scipy.optimize.fmin(fx, guess, disp=False, full_output=True)[
                0:2
            ]  # is this correct??
            # convert to float
        ...
        # Save data
        if xtr == "min":
            self._xmin, self._ymin = (+x_xtr, +y_xtr)
        if xtr == "max":
            self._xmax, self._ymax = (+x_xtr, -y_xtr)

    def get_min(self):
        return float(self._xmin), float(self._ymin)

    def get_max(self):
        return float(self._xmax), float(self._ymax)
...
class VadiSpline(Spline):
    ...
    def get_saddle(self):
        return float(self._ssad), float(self._Vsad)

    def get_omega(self):
        return self.get_last()

    def get_AG(self):
        return float(self._sAG), float(self._VAG)

    def relative(self, si, E=0.0):
        return self(si) - E
